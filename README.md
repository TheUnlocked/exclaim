...
* Some features (debouncing vs throttling, global `$context.follow`) are built with the assumption that the bot will only be used in smaller contexts (well within standard rate limits on average). This may be a fair assumption given the scope of Exclaim!, but it should be considered and stated explicitly if so.
